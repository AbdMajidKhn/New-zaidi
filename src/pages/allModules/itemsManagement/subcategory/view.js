...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(Number(store.data.itemsManagement.subcategory.perPage));
	...
	const getEditingItem = (idd) => {
		...
		apiClient.get(`/editSubCategories?id=${idd}`).then((res) => {
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'itemsManagement', 'subcategory', 'perPage']));
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'itemsManagement', 'subcategory', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.itemsManagement.subcategory.tableData?.data?.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.itemsManagement.subcategory.tableData?.from ?? 1}
					to={store.data.itemsManagement.subcategory.tableData?.to ?? 1}
					total={store.data.itemsManagement.subcategory.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.itemsManagement.subcategory?.pageNo ?? 1}
							totalItemsCount={
								store.data.itemsManagement.subcategory?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.itemsManagement.subcategory?.perPage ?? 10,
							)}
