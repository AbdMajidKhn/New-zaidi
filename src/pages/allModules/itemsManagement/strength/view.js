...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(Number(store.data.itemsManagement.strength.perPage));
	...
	const getEditingItem = (idd) => {
		...
		apiClient.get(`/editStrengthUnits?id=${idd}`).then((res) => {
			setEditingItem(res.data.StrengthUnit);
	...
	const deleteItem = (id) => {
		apiClient
			.delete(`/deleteStrengthUnits?id=${id}`)
			.then((res) => {
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'itemsManagement', 'strength', 'perPage']));
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'itemsManagement', 'strength', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.itemsManagement.strength.tableData?.data.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.itemsManagement.strength.tableData?.from ?? 1}
					to={store.data.itemsManagement.strength.tableData?.to ?? 1}
					total={store.data.itemsManagement.strength.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.itemsManagement.strength?.pageNo ?? 1}
							totalItemsCount={
								store.data.itemsManagement.strength?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.itemsManagement.strength?.perPage ?? 10,
							)}
						...
						<CardHeader>
							<CardLabel icon='Delete' iconColor='info'>
								<CardTitle>
									...
									<small> Strength Id: {itemId}</small>
								</CardTitle>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
					...
					<div className='row g-4'>
						<div className='col-12'>
							<Card>
								<CardBody>
									<h5>
										Are you sure, you want to delete the selected Strength?{' '}
										<br />
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Strength</CardTitle>
								<small> Strength Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
