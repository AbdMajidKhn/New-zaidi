...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(Number(store.data.itemsManagement.category.perPage));
	...
	const getEditingItem = (idd) => {
		...
		apiClient.get(`/editCategory?id=${idd}`).then((res) => {
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'itemsManagement', 'category', 'perPage']));
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'itemsManagement', 'category', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.itemsManagement.category.tableData?.data.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.itemsManagement.category.tableData?.from ?? 1}
					to={store.data.itemsManagement.category.tableData?.to ?? 1}
					total={store.data.itemsManagement.category.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.itemsManagement.category?.pageNo ?? 1}
							totalItemsCount={
								store.data.itemsManagement.category?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.itemsManagement.category?.perPage ?? 10,
							)}
