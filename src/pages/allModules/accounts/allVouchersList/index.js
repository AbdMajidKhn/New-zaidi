...
import GeneratePDF from '../viewAccounts/print/printGeneralJournal';
...
const DashboardPage = () => {
	...
	const print = (docType) => {
		try {
			GeneratePDF( docType, startDate2, endDate2);
		} catch (e) {}
	...
	const deleteFile = (id) => {
		apiClient
			.get(`/deleteVoucher?voucher_id=${id}`)
			.then((response) => {
				refreshVouchers();
		...
		<PageWrapper title={componentsMenu.components.subMenu.table.text}>
			<Page>
				<div className='row'>
					...
					<br />
									{/* <div className='col-md-2'>
										<FormGroup id='from_date' label='From'>
											<Input
												id='from_date'
												type='date'
												onChange={(e) => {
