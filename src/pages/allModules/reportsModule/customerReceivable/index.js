...
// import GeneratePDF from './customerReceivable';
...
// 		  apiClient
// 			.get(
// 			  `/getCustomerReceivebleBalance?customer_id=${formik.values.customer_id}&date=${formik.values.date || ''}`,
// 			)
// 			.then((response) => {
...
// 													setDateRange(dateStr); // Store the selected date as a string
...
const Files = () => {
	...
	const printReportAll = useCallback(() => {
		return apiClient
			.get(
				`/getCustomerReceivebleBalance?customer_id=${formik.values.customer_id}&date=${
					formik.values.date || ''
				}`,
			)
			.then((response) => {
    ...
    }, [formik.values.customer_id, formik.values.date, dispatch]);
				...
				<div className='row'>
					<div className='col-12'>
						<Card>
							...
							<View tableData={tableData} tableDataLoading={tableDataLoading} />
						</Card>
					</div>
				</div>
			</Page>
