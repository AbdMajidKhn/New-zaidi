...
const Excel = () => {
	const printReport = async () => {
		try {
			const response = await apiClient.get('/getSalesReport');
			const purchaseOrderList = response.data.invoices;
			const reportData = [];

			purchaseOrderList.forEach((purchaseOrder) => {
				purchaseOrder.invoice_child.forEach((item) => {
					const rowData = {
						pack: item.item.pack,
	...
	const handleOnExport = () => {
		const reportTitle = ['Sale Order Report']; // Wrap the title in an array
		printReport()
			.then((reportData) => {
				const table1 = [
					[
				...
				reportData.forEach((row) => {
					...
					const emptyRow = [''];
					const { totalAmount, discount, afterDiscount, tax, afterTax } = row;
					...
					const gst = ['', '', '', '', '', '', 'Tax:', tax];
					const afterGst = ['', '', '', '', '', '', 'Total After Tax:', afterTax];
					const emptyRow2 = [''];
					table1.push(
						emptyRow,
						totalAmountRow,
					...
					totalAfterDiscount += row.afterDiscount;
					totalGstAmount += row.tax;
					totalAmountAfterGst += row.afterTax;
				});
				...
				XLSX.utils.book_append_sheet(wb, sheet, 'Sale Report');
