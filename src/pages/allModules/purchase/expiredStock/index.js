...
const Categories = () => {
	...
	const refreshTableData = () => {
		...
		apiClient
			.get(
				`/getExpiredItemsInventory?records=${
					store.data.purchase.inventory.perPage
				}&pageNo=${store.data.purchase.inventory.pageNo}&colName=id&sort=desc
				&subcategory_id=${selectedsubcategory ? selectedsubcategory.id : ''}&category_id=${
					selectedcategory ? selectedcategory.id : ''
				}&manufacture_id=${selectedmanufacturer ? selectedmanufacturer.id : ''}&item_id=${
					selecteditem ? selecteditem.id : ''
				}&batch_no=${batchNo || ''}&expiry_from=${dateRange.from || ''}&expiry_to=${
					dateRange.to || ''
				}`,
				{},
			)
