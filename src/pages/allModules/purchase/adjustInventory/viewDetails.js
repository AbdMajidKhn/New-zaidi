			...
			<div className='row g-4 mt-2'>
				<div className='col-md-12'>
					<table className='table table-modern col-md-12'>
						...
						<tbody>
							{viewItem?.adjustInventoryChild?.map((item) => {
								// const correspondingChild = viewItem?.adjustInventoryChild?.find(
								// 	(child) => child.item_id === item.item_id
								// );

								return (
									<tr key={item.id}>
										{console.log('meter size', item)}
										...
										<td>{item.batch_no}</td>
										<td>{item.pack}</td>
										{/* <td>{correspondingChild?.meter_size?.meter_size_dropdown?.name}</td> */}
										{/* <td>{correspondingChild?.thaan}</td> */}
										<td>
