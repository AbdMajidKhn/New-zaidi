...
const AdjustInventory = ({ refreshTableData }) => {
	...
	useEffect(() => {
		apiClient.get(`/getItemMeterSize?item_id=${itemId}`)
			.then((response) => {
												...
												<tr className='row' key={items.index}>
													<td className='col-md-3'>
														...
														{formik.errors[
															...
															<FormGroup label='Meter Size' id={`childArray[${index}].meter_size`}>
																<ReactSelect
																	classNamePrefix='select'
																	options={meterSizeOptions.map(option => ({
																		id: option.meter_size_id,
																		value: option.meter_size_dropdown.id,
																		label: option.meter_size_dropdown.name
																	}))}
																	value={items.meter_size ? items.meterSizeOptions?.find(c => c.value === items.meter_size) : null}
																	onChange={(val) => handleMeterSizeChange(val, index)}
																/>
															</FormGroup>
															{formik.errors.meterSize && (
