...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [editingItemChild, setEditingItemChild] = useState({});
	const [batchDetails, seBatchDetails] = useState({});
	...
	const getEditingItem = (idd) => {
		setEditingItemLoading(true);
		apiClient.get(`/editAdjustItemInventory?id=${idd}`).then((res) => {
			console.log('responseedit',res)
			setEditingItem(res.data.adjustInventory);
			setEditingItemChild(res.data.adjustInventoryChild);
			seBatchDetails(res.data.batchQuantity);
			setEditingItemLoading(false);
		});
	};
	const handleStateEdit = (status) => {
	...
	let todayDate = formik.initialValues.date;
					...
					<div className='row g-4'>
						<div className='col-12'>
							{editingItemLoading ? (
								...
								<Edit editingItem={editingItem} 
								editingItemChild={editingItemChild}
