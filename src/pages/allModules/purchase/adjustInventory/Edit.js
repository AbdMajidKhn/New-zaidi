...
const Edit = ({ editingItem, handleStateEdit, editingItemChild }) => {
		...
		if (val) {
			apiClient.get(`/getbatchNo?manufacture_id=${formik.values.adjustInventoryChild[idx]?.manufacture.id}&item_id=${formik.values.adjustInventoryChild[idx]?.item_id}`)
				.then((response) => {
					...
					if (selectedBatch) {
						formik.setFieldValue(`adjustInventoryChild[${idx}].batchQty`, selectedBatch.item_available);
					}
						...
						<tbody>
							{formik.values.adjustInventoryChild?.length > 0 && (
								formik.values.adjustInventoryChild.map((items, index) => {
									const adjustInventoryChild = formik.initialValues.adjustInventoryChild?.[index] || {};
									return (
										...
										<tr className='row' key={items.index}>
													...
													{formik.values.adjust_type === 'add' ? (
														...
														<ReactSelect
															...
															onChange={(val) => {
																...
																formik.setFieldValue(`adjustInventoryChild[${index}].batchQuantity[0].item_available`, val.item_available);
															}}
															isValid={formik.isValid}
															isTouched={formik.touched.adjustInventoryChild ? formik.touched.adjustInventoryChild[index]?.batch_no : ''}
															invalidFeedback={formik.errors[`adjustInventoryChild[${index}].batch_no`]}
											...
											<td className='col-md-1'>
												...
												{formik.values.adjust_type === 'remove' ? <p className='fs-6'>
													Batch qty{' '}
													{items.batchQuantity[0]?.item_available}
												...
												<FormGroup
													...
													<Input
														...
														onChange={(val) => {
															...
															const isValidNumber = !isNaN(inputValue) && (inputValue === '' || /^-?\d+(\.\d+)?$/.test(inputValue));

															if (isValidNumber) {
																formik.setFieldValue(`adjustInventoryChild[${index}].purchase_price`, inputValue);
															}

															// Increment trigger for calculating total
															setTriggerCalculateTotal(triggerCalculateTotal + 1);
														}}
														onBlur={formik.handleBlur}
														...
														isTouched={
															formik.touched.adjustInventoryChild
																? formik.touched.adjustInventoryChild[index]?.purchase_price
																: ''
														}
														invalidFeedback={
															formik.errors[`adjustInventoryChild[${index}].purchase_price`]
														}
													/>
...
// // eslint-disable-next-line react/prop-types
// const Edit = ({ editingItem, handleStateEdit, editingItemChild }) => {
// 	console.log('editingItem', editingItem, editingItemChild)
// 	// console.log('editingItemChild', editingItemChild)
// 	const [isLoading, setIsLoading] = useState(false);
// 	const [lastSave, setLastSave] = useState(null);
// 	const [supplierDropDown, setSupplierDropDown] = useState([]);
...
// 				}),
// 			);
// 			const manufacturer = response.data.items[0].manufacturerOptions.map((d) => ({
// 				id: Number(d.id),
// 				value: Number(d.id),
// 				label: d.label,
// 			}));
...
// 				formik.setFieldValue(
// 					`adjustInventoryChild[${index}].manufactureOptions`,
// 					selectedItem.manufactureOptions
// 				);
// 				formik.setFieldValue(
// 					`adjustInventoryChild[${index}].item_name`,
// 					selectedItem.label
...
// 			}
// 		});
// 	}, [formik.values.adjustInventoryChild.map(item => item.item_id).join(), itemOptions]);

// 	// useEffect(() => {
// 	// 	const uniqueManufacturerIds = new Set(
// 	// 		formik.values.adjustInventoryChild
// 	// 			.map(item => item.manufacturer.id)
// 	// 			.filter(id => id)
// 	// 	);

// 	// 	if (uniqueManufacturerIds.size > 0) {
// 	// 		const manufacturerIds = Array.from(uniqueManufacturerIds);

// 	// 		apiClient.get(`/getBatchOptions?manufacturer_ids=${manufacturerIds.join(',')}`)
// 	// 			.then((response) => {
// 	// 				const batchOptionsMap = new Map();
// 	// 				response.data.batches.forEach(({ manufacturer_id, id, label }) => {
// 	// 					if (!batchOptionsMap.has(manufacturer_id)) {
// 	// 						batchOptionsMap.set(manufacturer_id, []);
// 	// 					}
// 	// 					batchOptionsMap.get(manufacturer_id).push({
// 	// 						id,
// 	// 						value: id,
// 	// 						label,
// 	// 					});
// 	// 				});
// 	// 				formik.setFieldValue(`adjustInventoryChild[${idx}].batchDetails`, batchOptionsMap);

// 	// 				// Update formik field values with batch options
// 	// 				formik.values.adjustInventoryChild.forEach((item, index) => {
// 	// 					const batchOptions = batchOptionsMap.get(item.manufacturer.id) || [];
// 	// 					formik.setFieldValue(
// 	// 						`adjustInventoryChild[${index}].batchOptions`,
// 	// 						batchOptions
// 	// 					);
...
// 	// 			});
// 	// 	}
// 	// }, [formik.values.adjustInventoryChild.map(item => item.manufacturer.id).join()]);
...
// 	// 				data.quantity_in = data.thaan * data.meter_size.name
// 	// 				data.amount = data.quantity_in * data.purchase_price;
// 	// 			})
// 	// 			: arr?.forEach((data) => {
// 	// 				data.quantity_out = data.thaan * data.meter_size.name
...
// 			apiClient.get(`/getbatchNo?manufacture_id=${formik.values.adjustInventoryChild[0]?.manufacture.id}`).then((response) => {
// 				console.log(response, 'respbatch')
...
// 				formik.setFieldValue(`adjustInventoryChild[${idx}].batchDetails`, batchOptions);
...
// 															classNamePrefix='select'
// 															options={items.batchDetails}
// 															value={formik.values.adjustInventoryChild[index].batch_no ? items.batchDetails?.find(
// 																(c) => c.label === formik.values.adjustInventoryChild[index].batch_no
// 															) : null}
// 															onChange={(val) => {
// 																formik.setFieldValue(`adjustInventoryChild[${index}].batch_no`, val ? val.label : '');
// 																formik.setFieldValue(`adjustInventoryChild[${index}].expiry_date`, val.batchExpiry);
...
// 														}}
// 														onBlur={formik.handleBlur}
// 														value={items.quantity_in ? items.quantity_in : items.quantity_out}
// 														isValid={formik.isValid}
// 														isTouched={
// 															formik.touched.adjustInventoryChild
// 																? formik.touched.adjustInventoryChild[
...
// 											<td className='col-md-2'>
// 												<FormGroup
// 													id={`adjustInventoryChild[${index}].rate`}
// 													label=''
// 													className='col-md-12'>
// 													<Input
// 														onWheel={(e) => e.target.blur()}
...
// 														onChange={(val) => {
// 															formik.setFieldValue(
// 																`adjustInventoryChild[${index}].rate`,
// 																val.target.value,
// 															);
// 															setTriggerCalculateTotal(
// 																triggerCalculateTotal + 1,
