...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(Number(store.data.storeManagement.store.perPage));
	...
	const getEditingItem = (idd) => {
		...
		apiClient.get(`/editStore?id=${idd}`).then((res) => {
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'storeManagement', 'store', 'perPage']));
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'storeManagement', 'store', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.storeManagement.store.tableData?.data?.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='Store'
					from={store.data.storeManagement.store.tableData?.from ?? 1}
					to={store.data.storeManagement.store.tableData?.to ?? 1}
					total={store.data.storeManagement.store.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.storeManagement.store?.pageNo ?? 1}
							totalItemsCount={
								store.data.storeManagement.store?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.storeManagement.store?.perPage ?? 10,
							)}
