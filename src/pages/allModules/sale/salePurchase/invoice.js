...
const validate = (values) => {
	...
	values.childArray.forEach((data, index) => {
		...
		if (!data.remarks) {
			errors = {
				...errors,
				[`childArray[${index}]remarks`]: 'Required',
			};
		}
		if (!data.quantity > 0) {
			errors = {
...
const Invoice = ({ editingItem, handleStateReceive }) => {
	...
	const formik = useFormik({
		initialValues: { ...editingItem, date: todayDate, tax_type: 1, adv_tax_type: 1 },
		validate,
		onSubmit: () => {
			setIsLoading(true);
	...
	const submitForm = (data) => {
		apiClient
			.post(`/generateInvoiceSalePo`, data)
			.then((res) => {
		...
		<div className='col-12'>
			<Card stretch tag='form' onSubmit={formik.handleSubmit}>
				<CardBody>
								...
								formik.values.childArray.map((items, index) => (
									<tr className='row' key={items.index}>
										...
										<td className='col-md-2 border-start border-end '>
											...
											<p className='fs-6'>
												Batch Exp{' '}
												{items.batchDetails?.batchExpiry ?? 'None'}
											</p>
										</td>
										<td className='col-md-2 border-start border-end '>
											...
											{/* <Popovers desc='This is a title' trigger='hover'>
												<p className='fs-6'>
													Quoted price {items.quoted_rate ?? 'None'}
												</p>
											</Popovers> */}
					...
					<div className='row g-4 d-flex align-items-top mt-5'>
						<div className='col-md-3'>
							<FormGroup
								id='amount_received'
