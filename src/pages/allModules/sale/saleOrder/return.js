...
const validate = (values) => {
	...
	values.childArray.forEach((data, index) => {
		...
		if (Number(data.quantity) + Number(data.returned_qty) > data.sold_quantity) {
			errors = {
				...errors,
				[`childArray[${index}]quantity`]: ' Return Quantity Exceeded',
			};
		}
...
const Edit = ({ editingItem, handleStateReturn }) => {
	...
	useEffect(() => {
		...
		// console.log(formik.values.total_after_adv_tax);
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [formik.values.adv_tax_percentage]);

	// useEffect(() => {
	// 	const calculateTotalAfterAdvTax = () => {
	// 	  const advTaxAmount = (formik.values.total_after_gst * formik.values.adv_tax_percentage) / 100;
	// 	  formik.setFieldValue('adv_tax', advTaxAmount);
	// 	  const totalAfterAdvTax = formik.values.total_after_gst + advTaxAmount;
	// 	  formik.setFieldValue('total_after_adv_tax', totalAfterAdvTax);
	// 	};

	// 	calculateTotalAfterAdvTax();
	//   }, [formik.values.total_after_gst, formik.values.adv_tax_percentage]);

	useEffect(() => {
		// Initialize total_after_adv_tax with total_after_gst
		formik.setFieldValue('total_after_adv_tax', formik.values.total_after_gst);

		const calculateTotalAfterAdvTax = () => {
			const advTaxAmount =
				(formik.values.total_after_discount * formik.values.adv_tax_percentage) / 100;
			formik.setFieldValue('adv_tax', advTaxAmount);
			// console.log(advTaxAmount);
			const totalAfterAdvTax = formik.values.total_after_gst + advTaxAmount;
			formik.setFieldValue('total_after_adv_tax', totalAfterAdvTax);
		};
			...
			<Card stretch tag='form' onSubmit={formik.handleSubmit}>
				<CardBody>
					...
					<div className='row g-2  d-flex justify-content-start'>
						<div className='col-md-2'>
							<FormGroup id='invoice_no' label='Invoice No' className='col-md-12'>
								<Input
								...
								formik.values.childArray.map((items, index) => (
									<tr className='row' key={items.index}>
										...
										<td className='col-md-2 border-start border-end '>
											...
											<div className='fs-6'>
												Ret Qty {items.returned_qty ?? 'None'}
											</div>
											<div className='fs-6'>
												Remained Qty{' '}
												{items.sold_quantity - items.returned_qty ?? 'None'}
											</div>
										</td>
										<td className='col-md-2 border-start border-end '>
				...
				<CardFooter>
					...
					<CardFooterRight>
						<Button
							className='me-3'
							icon={isLoading ? null : 'Confirm Return'}
							isLight
							color='success'
							isDisable={isLoading}
