...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(Number(store.data.quotations.quotation.perPage));
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'quotations', 'quotation', 'perPage']));
		},
		// eslint-disable-next-line react-hooks/exhaustive-deps
		[perPage],
	);
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'quotations', 'quotation', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.quotations.quotation.tableData?.data?.map((item, index) => (
								<tr key={item.id}>
									<td>
										<Checks
											id={item.id.toString()}
				...
				<PaginationButtons
					label='Sales'
					from={store.data.quotations.quotation.tableData?.from ?? 1}
					to={store.data.quotations.quotation.tableData?.to ?? 1}
					total={store.data.quotations.quotation.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.quotations.quotation?.pageNo ?? 1}
							totalItemsCount={store.data.quotations.quotation?.tableData?.total ?? 0}
							itemsCountPerPage={Number(
								store.data.quotations.quotation?.perPage ?? 10,
							)}
